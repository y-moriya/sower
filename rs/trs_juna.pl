...
'　……そして、その日、村には新たなルールが付け加えられた。<br>　見分けの付かない人狼を排するため、1日1人ずつ疑わしい者を処刑する。誰を処刑するかは全員の投票によって決める……<br>　無辜の者も犠牲になるが、やむを得ない……<br><br>　そして、人間と人狼の暗く静かな戦いが始まった。',
    ...
    # 処刑時のお知らせ
    my @announce_vote = (
        '_NAME_ は _TARGET_ に投票した_RANDOM_',
        '_NAME_ に _COUNT_人が投票した',
        '_NAME_ は村人の手により処刑された……'
    );
    ...
    my @announce_totalcommit = (
        ...
        ,    # 0～1/3の時
        '「時間を進める」を選択している人は全体の1/3から2/3の間のようです。'
        ,    # 1/3～2/3の時
...
'その時、人狼は勝利を確信し、そして初めて過ちに気づいた。<br>しかし、天敵たる妖魔を討ち漏らした人狼には、最早なすすべがなかった……',
    ...
    # 勝利者
    my @caption_winner = ( '', '村人', '人狼', '妖魔', '妖魔', );

    # 役職名
    my @rolename = (
        'おまかせ', '村人',    '人狼', '占い師',    '霊能者', '狂人',
        '守護者',    '結社員', '妖魔', '囁き狂人', '聖痕者', '狂信者',
        '共鳴者',    '天魔',    '呪狼', '智狼',       '悪戯妖精',
    );
...
'<p>あなたは悪戯妖精です。<br>人狼に殺されることがありません。ただし、占いの対象となると死亡します。<br>霊能者には人間として判別されますが、勝利条件では村人にも人狼にも数えられません。<br>悪戯妖精は１日目、好きな二人に“運命の絆”を結びつける事ができます。“運命の絆”を結んだ人は、片方が死亡すると後を追って死亡します。<br>人狼が全滅するか、村人(妖魔と天魔と悪戯妖精を除く)の数が人狼と同数以下まで減るまで「生き残れば」勝利です。</p>',
    ...
    my %explain_roles = (
        prologue =>
'あなたは_SELROLE_を希望しています。ただし、希望した通りの能力者になれるとは限りません。',
    ...
    my %textrs = (
        ...
        EXITMES =>
          '_NAME_がいたような気がしたが、気のせいだったようだ……(_NAME_は村を出ました)',
...
'_AVICTORY_全てのログとユーザー名を公開します。_DATE_ まで自由に書き込めますので、今回の感想などをどうぞ。',
        ...
        ANNOUNCE_SETENTRUST =>
          '投票を委任します。<br><br>_NAME_は、_TARGET_ に投票を委任しました。',
