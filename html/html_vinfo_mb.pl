...
sub OutHTMLSayFilterPlayersMb {
	...
	foreach (@filterlist) {
	    ...
	    my $showid = "";
	    $showid = " ($_->{'uid'})" if ($vil->isepilogue() != 0 || ($vil->{'showid'} > 0) || ($sow->{'uid'} eq $sow->{'cfg'}->{'USERID_ADMIN'}));
		my $rolename = '';
		$rolename = " [$sow->{'textrs'}->{'ROLENAME'}->[$_->{'role'}]]" if ($_->{'role'} > 0);
		my $live = 'live';
		$live = $sow->{'curpl'}->{'live'} if (defined($sow->{'curpl'}->{'live'}));
	    my $showall = "";
	    $showall = " $rolename" if ($vil->isepilogue() != 0 || ($vil->{'showall'} > 0 && $live ne 'live') || ($sow->{'uid'} eq $sow->{'cfg'}->{'USERID_ADMIN'}));
		#if ($vil->{'turn'} == 0) {
		#	print "$chrname";
		#} else {
			print "<a href=\"$urlsow?$linkvalue$amp" . "pno=$_->{'pno'}\">$chrname</a>$showid$showall [<a href=\"$urlsow?$linkvalue$amp" . "pno=$_->{'pno'}$amp" . "cmd=mbimg\">顔</a>]";
		#}
		print " ($_->{'deathday'}d)" if ($livetype ne 'live');
		if (($_->{'live'} eq 'live') || ($live ne 'live') || ($vil->isepilogue() != 0)) {
