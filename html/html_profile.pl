...
_HTML_
        ...
        foreach (@list) {
            ...
            $vil->readvil();
            my $linkvalue = &SWBase::GetLinkValues( $sow, $reqvals );
            print "<li><a href=\"$urlsow?$linkvalue#newinfo\">$_->{'vid'}村 $vil->{'vname'}</a> $_->{'chrname'}</li>\n";
            $vil->closevil();
        }
...
_HTML_
        ...
        foreach (@list) {
            ...
            print "<li><a href=\"$urlsow?$linkvalue#newinfo\">$_->{'vid'} $_->{'vname'}</a><br$net>【"
              . $winstr[ $_->{'win'} + 1 ]
...
_HTML_
        ...
        foreach (@list) {
            ...
            $reqvals->{'vid'}  = $_->{'vid'};
            my $linkvalue = &SWBase::GetLinkValues( $sow, $reqvals );
            print "<li><a href=\"$urlsow?$linkvalue#newinfo\">$_->{'vid'}村</a>：$lovertext と愛の絆を結んでいた。</li>\n";
            $lovercount++;
        }
...
_HTML_
    ...
    for ( $i = 1 ; $i < @$data ; $i++ ) {
        ...
        next if ( $title eq '陣営' && $i == 4 );

        my $side = '';
        $side = '側' if ( $title eq '陣営' );
        if ( $data->[$i]->{'total'} > 0 ) {
