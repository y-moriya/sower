...
_HTML_
	...
	if (1) {
		my $penaltydt = int(($user->{'penaltydt'} - $sow->{'time'}) / 60 / 60 / 24 + 0.5);
		my @penalty = (
			"なし",
...
_HTML_
		...
		foreach (@list) {
			$reqvals->{'vid'} = $_->{'vid'};
			my $linkvalue = &SWBase::GetLinkValues($sow, $reqvals);
			print "<li><a href=\"$urlsow?$linkvalue#newsay\">$_->{'vid'}村 $_->{'vname'}</a> $_->{'chrname'} $_->{'playing'}</li>\n";
		}
...
_HTML_
		...
		foreach (@list) {
			...
			print "<li><a href=\"$urlsow?$linkvalue#newsay\">$_->{'vid'} $_->{'vname'}</a><br$net>【" . $winstr[$_->{'win'} + 1] . "】 $_->{'chrname'}$rolenametext、$liveday$livetext{$_->{'live'}}</li>\n";
		}
...
_HTML_
		...
		my $vscount = 0;
		my @vskeys =  keys(%vs);
		my @vs = sort {$vs{$b}->{'total'} <=> $vs{$a}->{'total'} or $vs{$b}->{'win'} <=> $vs{$a}->{'win'} or $vs{$b}->{'draw'} <=> $vs{$a}->{'draw'} or $vs{$b}->{'lose'} <=> $vs{$a}->{'lose'}} @vskeys;
		foreach (@vs) {
			next if ($vs{$_}->{'total'} < $sow->{'cfg'}->{'MIN_VSRECORDTOTAL'});
			print "<li>vs. <a href=\"$vs{$_}->{'url'}\">$vs{$_}->{'uid'}</a> $vs{$_}->{'total'}戦 $vs{$_}->{'win'}勝 $vs{$_}->{'lose'}敗 $vs{$_}->{'draw'}分</li>\n";
			$vscount++;
		}
...
_HTML_
		...
		for ($i = 1; $i < @$data; $i++) {
			if ($data->[$i]->{'total'} > 0) {
				my ($average, $liveaverage, $livedays) = &SetRecordText($data->[$i]);
				print <<"_HTML_";
<tr>
  <td>$caption->[$i]側</td>
