...
_HTML_
        ...
        foreach (@$vilist) {
            next
              if ( ( $_->{'vstatus'} ne $sow->{'VSTATUSID_PLAY'} )
                && ( $_->{'vstatus'} ne $sow->{'VSTATUSID_EP'} )
                && ( $_->{'vstatus'} ne $sow->{'VSTATUSID_SCRAP'} ) );
            $vicount += &OutHTMLVIndexSingleMb( $sow, $_ );
        }
        if ( $vicount == 0 ) {
            print "なし。<br$net>";
...
_HTML_
        ...
        foreach (@$vilist) {
            next
              if ( ( $_->{'vstatus'} ne $sow->{'VSTATUSID_END'} )
                && ( $_->{'vstatus'} ne $sow->{'VSTATUSID_SCRAPEND'} ) );
            $virow++;
            if ( $maxrow > 0 ) {
