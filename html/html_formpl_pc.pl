...
sub OutHTMLPlayerFormPC {
    ...
    if (   ( ( $curpl->iswhisper() == 0 ) || ( $query->{'mode'} ne 'whisper' ) )
        || ( $vil->isepilogue() != 0 ) )
    {
        # 発言欄HTML出力
...
_HTML_
    ...
    $caption_say = $sow->{'textrs'}->{'CAPTION_GSAY_PC'}
    ...
    $draft = 1
      if (
        ( $sow->{'savedraft'} ne '' )
        && (   ( $sow->{'draftmestype'} == $sow->{'MESTYPE_SAY'} )
            || ( $sow->{'draftmestype'} == $sow->{'MESTYPE_TSAY'} ) )
      );

    if (
        ( $query->{'mes'} ne '' )
        && (   ( $query->{'cmdfrom'} eq 'write' )
            || ( $query->{'cmdfrom'} eq 'writepr' ) )
      )
    {
        my $mes = $query->{'mes'};
        $mes =~ s/<br( \/)?>/\n/ig;
...
_HTML_
            ...
            $draft = 1
              if (
                ( $sow->{'savedraft'} ne '' )
                && (   ( $sow->{'draftmestype'} == $sow->{'MESTYPE_WSAY'} )
                    || ( $sow->{'draftmestype'} == $sow->{'MESTYPE_SPSAY'} )
                    || ( $sow->{'draftmestype'} == $sow->{'MESTYPE_BSAY'} ) )
              );
...
<div class="formpl_gm">
  ...
  <p class="commitbutton">
    ...
    <input type="submit" value="雑談村を有効化する" data-submit-type="chatmode"$net>
  </p>
  </form>
</div>
...
sub OutHTMLVilMakerPC {
    ...
    $imgpl{'deathday'} = $curpl->{'deathday'}
      if ( defined( $curpl->{'deathday'} ) );
