...
_HTML_
	...
	if ($vil->{'makeruid'} eq $sow->{'uid'}) {
		&OutHTMLVilMakerMb($sow, $vil, 'maker') if (($vil->{'turn'} == 0) || ($vil->isepilogue() != 0) || ($vil->{'makersaymenu'} == 0));
		# 開始、編集とキックボタンは後回し。
...
_HTML_
	...
	# テキストボックスと発言ボタン
	my $caption_say = $sow->{'textrs'}->{'CAPTION_SAY_MB'};
	$caption_say = $sow->{'textrs'}->{'CAPTION_GSAY_MB'} if (($curpl->{'live'} ne 'live') && ($vil->isepilogue() == 0));
	my $buttonlabel = $sow->{'textrs'}->{'BUTTONLABEL_MB'};
	$buttonlabel =~ s/_BUTTON_/$caption_say/g;
	...
	my $mes = '';
	$mes = $query->{'mes'} if (($query->{'wolf'} eq '') && ($query->{'maker'} eq '') && ($query->{'admin'} eq '') && ($query->{'guest'} eq ''));
	$mes =~ s/<br( \/)?>/\n/ig;
	print <<"_HTML_";
<textarea name="mes" rows="3" istyle="1">$mes</textarea><br$net>
	...
	if (($curpl->{'live'} eq 'live') || ($cfg->{'ENABLED_TSAY_GRAVE'} > 0)) { # 生きている／墓下独り言有効
		if (($vil->isepilogue() == 0) || ($cfg->{'ENABLED_TSAY_EP'} > 0)) { # エピではない／エピ独り言有効
			if (($vil->{'turn'} != 0) || ($cfg->{'ENABLED_TSAY_PRO'} > 0)) {
				...
				my $checked = '';
				$checked = " $sow->{'html'}->{'checked'}" if (($query->{'think'} ne '') && ($query->{'guest'} eq ''));
				print "<input type=\"checkbox\" name=\"think\" value=\"on\"$checked$net>$sow->{'textrs'}->{'CAPTION_TSAY_MB'} あと$curpl->{'tsay'}$unit\n";
			}
		}
	}

	my $checked = '';
	$checked = " $sow->{'html'}->{'checked'}" if (($query->{'monospace'} ne '') && ($query->{'guest'} eq ''));
	print "<input type=\"checkbox\" name=\"monospace\" value=\"on\"$checked$net>等幅\n";
	my $checked = '';
	$checked = " $sow->{'html'}->{'checked'}" if (($query->{'loud'} ne '') && ($query->{'guest'} eq ''));
...
_HTML_
	...
	# コミットボタン
	&OutHTMLCommitFormMb($sow, $vil) if (($vil->{'turn'} > 0) && ($vil->isepilogue() == 0) && ($sow->{'curpl'}->{'live'} eq 'live'));

	# 村を出る（暫定）
	OutHTMLExitVilButtonMb($sow, $vil) if ($vil->{'turn'} == 0);

	# アクション
	&OutHTMLActionFormMb($sow, $vil) if ((($curpl->{'live'} eq 'live') || ($vil->isepilogue() != 0)) && (($vil->{'noactmode'} == 0) || ($vil->{'noactmode'} == 2)));
...
sub OutHTMLGuestMb {
	my ($sow, $vil) = @_;
	return if (($vil->{'turn'} > 0) && ($vil->checkentried() > 0) && ($vil->isepilogue() == 0));
	my $query = $sow->{'query'};
	my $cfg   = $sow->{'cfg'};
	my $net = $sow->{'html'}->{'net'};
...
_HTML_
	...
	# テキストボックスと発言ボタン
	my $caption_say = $sow->{'textrs'}->{'CAPTION_SAY_MB'};
	$caption_say = $sow->{'textrs'}->{'CAPTION_GSAY_MB'} if (($vil->{'turn'} > 0) && ($vil->isepilogue() == 0));
	my $buttonlabel = $sow->{'textrs'}->{'BUTTONLABEL_MB'};
	$buttonlabel =~ s/_BUTTON_/$caption_say/g;
...
_HTML_
	...
	if (($vil->isepilogue() == 0) || ($cfg->{'ENABLED_TSAY_EP'} > 0)) { # エピではない／エピ独り言有効
		my $checked = '';
		$checked = " $sow->{'html'}->{'checked'}" if (($query->{'think'} ne '') && ($query->{'guest'} ne ''));
		print "<input type=\"checkbox\" name=\"think\" value=\"on\"$checked$net>$sow->{'textrs'}->{'CAPTION_TSAY_MB'}\n";
	}

	my $checked = '';
	$checked = " $sow->{'html'}->{'checked'}" if (($query->{'monospace'} ne '') && ($query->{'guest'} ne ''));
	print "<input type=\"checkbox\" name=\"monospace\" value=\"on\"$checked$net>等幅\n";
	my $checked = '';
...
_HTML_
	} elsif (($vil->isepilogue() != 0) || ($sow->{'curpl'}->{'live'} ne 'live')) {
		# 能力欄表示（墓下／エピローグ）
		my $mes = $role->{'explain'};
...
_HTML_
			...
			my $checked = '';
			$checked = " $sow->{'html'}->{'checked'}" if ($query->{'monospace'} ne '');
			print "<input type=\"checkbox\" name=\"monospace\" value=\"on\"$checked$net>等幅\n";
