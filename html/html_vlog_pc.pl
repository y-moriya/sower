...
sub OutHTMLVlogPC {
	...
	# ログID指定表示スイッチ
	my $modesingle = 0;
	$modesingle = 1 if (($query->{'logid'} ne '') || ($query->{'ua'} eq 'mb')); # 不具合あったりしないよね？
	...
	if (($query->{'order'} eq 'desc') || ($query->{'order'} eq 'd')){
		...
		for ($i = 0; $i < @$logs; $i++) {
			...
			if (($modesingle == 0) && ($i == 0) && (($maxrow != 0) && ($rows->{'rowover'} > 0))) {
				...
				print "<p id=\"readmore\" class=\"readmore\"><a href=\"$link&logid=$logid&move=prev\" onclick=\"getMoreLog(this);return false;\">もっと読む</a><img id=\"morelog-ajax-loader\" src=\"$cfg->{'DIR_IMG'}/ajax-loader.gif\" style=\"display: none;\"></p>\n\n";
			}
			&SWHtmlVlogSinglePC::OutHTMLSingleLogPC($sow, $vil, $log, $i, $newsay, \%anchor, $modesingle);
			if ($i == $#$logs) {
				my $logid = $log->{'logid'};
...
sub OutHTMLVilMakerInFormPlPC {
	...
	if ($vil->{'makeruid'} eq $sow->{'uid'}) {
		require "$cfg->{'DIR_HTML'}/html_formpl_pc.pl";
		print "<div class=\"formpl_frame\">\n";
		&SWHtmlPlayerFormPC::OutHTMLVilMakerPC($sow, $vil, 'maker') if (($vil->{'turn'} == 0) || ($vil->isepilogue() != 0) || ($vil->{'makersaymenu'} == 0));
		&SWHtmlPlayerFormPC::OutHTMLUpdateSessionButtonPC($sow, $vil) if ($vil->{'turn'} == 0);
		&SWHtmlPlayerFormPC::OutHTMLKickFormPC($sow, $vil) if ($vil->{'turn'} == 0);
		print "</div>\n";
	}
