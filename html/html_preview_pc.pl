...
_HTML_
	...
	# 誤爆注意
	if ((&SWBase::CheckWriteSafetyRole($sow, $vil) > 0) && ($log->{'mestype'} == $sow->{'MESTYPE_SAY'}) && ($vil->isepilogue() == 0)) {
		print <<"_HTML_";
...
_HTML_
	...
	my @reqkeys = ('csid_cid', 'role', 'mes', 'think', 'wolf', 'maker', 'admin', 'sympathy', 'werebat', 'monospace', 'expression', 'guest', 'loud');
	...
	if ($lineslogcount > $saycnt->{'MAX_MESLINE'}) {
		...
		my $unitcaution = $sow->{'basictrs'}->{'SAYTEXT'}->{$sow->{'cfg'}->{'COUNTS_SAY'}->{$vil->{'saycnttype'}}->{'COUNT_TYPE'}}->{'UNIT_CAUTION'};
...
#	my $point = &SWBase::GetSayPoint($sow, $vil, $log->{'log'});
	...
	my $pointtext = '';
	if (($saycnt->{'COUNT_TYPE'} ne 'count') && ($sow->{'query'}->{'cmd'} ne 'entrypr') && (defined($sow->{'curpl'}->{$saytype}))) {
...
_HTML_
	...
	if (($preview->{'cmd'} ne 'entry') && ($query->{'admin'} eq '') && ($query->{'maker'} eq '') && ($query->{'guest'} eq '')) {
		print <<"_HTML_";
