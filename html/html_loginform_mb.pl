...
_HTML_
    ...
    my $mbsayimg = '';
    if ( $query->{'mbsayimg'} eq 'on' ) {
        $mbsayimg = " $sow->{'html'}->{'checked'}";
    }
    print <<"_HTML_";
</select> 表\示件数<br$net>
<select name="order">
...
<input type="checkbox" name="mbsayimg"$mbsayimg>顔画像を表\示する<br$net>
_HTML_
...
_HTML_
    ...
    if ( $sow->{'cfg'}->{'URL_BBS_MB'} eq '' ) {
        $urlbbs = $sow->{'cfg'}->{'URL_BBS'}
          if ( defined( $sow->{'cfg'}->{'URL_BBS'} ) );
        $namebbs = $sow->{'cfg'}->{'NAME_BBS'}
          if ( defined( $sow->{'cfg'}->{'NAME_BBS'} ) );
    }
    if ( $urlbbs ne '' ) {
        print <<"_HTML_";
<hr$net>
