...
_HTML_
    ...
    foreach (@$vilist) {
        ...
        $vstatusno = 'oldlog'
          if ( $vil->{'epilogue'} < $vil->{'turn'} );          # 終了した村
...
"<img src=\"$cfg->{'DIR_IMG'}/$rating->{'FILE'}\" width=\"$rating->{'WIDTH'}\" height=\"$rating->{'HEIGHT'}\" alt=\"[$rating->{'ALT'}]\" title=\"$rating->{'CAPTION'}\"$net> "
        ...
        print <<"_HTML_";
  <tr>
    <td>$imgpwdkey$imgrating<a href="$cfg->{'BASEDIR_CGI'}/$cfg->{'FILE_SOW'}?$link#newinfo">$_->{'vid'} $vil->{'vname'}</a> 〈<a href="$cfg->{'BASEDIR_CGI'}/$cfg->{'FILE_SOW'}?$linkvinfo#newinfo" title="$vcomment">情報</a>〉</td>
    <td>$plcnt</td>
    <td>$vstatus</td>
