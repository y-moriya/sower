...
_HTML_
    ...
    foreach (@csskey) {
        ...
        print "  <link rel=\""
          . $alternate
          . "stylesheet\" type=\"text/css\" href=\"$cfg->{'BASEDIR_DOC'}/$css->{$_}->{'FILE'}\" title=\"$css->{$_}->{'TITLE'}\"$net>\n";
    }

    # RSSの出力
    if ( ( $sow->{'html'}->{'rss'} ne '' ) && ( $cfg->{'ENABLED_RSS'} > 0 ) ) {
        print
"  <link rel=\"Alternate\" type=\"application/rss+xml\" title=\"RSS\" href=\"$sow->{'html'}->{'rss'}\"$net>\n";
    }
...
sub OutHTMLContentFrameHeader {
    ...
    $topbanner{'file'} = $css->{'FILE_TOPBANNER'}
      if ( defined( $css->{'FILE_TOPBANNER'} ) );
    $topbanner{'width'} = $css->{'WIDTH_TOPBANNER'}
      if ( defined( $css->{'WIDTH_TOPBANNER'} ) );
    $topbanner{'height'} = $css->{'HEIGHT_TOPBANNER'}
      if ( defined( $css->{'HEIGHT_TOPBANNER'} ) );
...
sub OutHTMLLogin {
    ...
    if ( $sow->{'user'}->logined() <= 0 ) {
        ...
        $disabled = " $sow->{'html'}->{'disabled'}"
          if ( ( $query->{'prof'} ne '' )
            || ( $query->{'cmd'} eq 'editprofform' )
            || ( $query->{'cmd'} eq 'editprof' ) );
...
_HTML_
        ...
        my $disabled = '';
        $disabled = " $sow->{'html'}->{'disabled'}"
          if ( ( $query->{'cmd'} eq 'entrypr' )
            || ( $query->{'cmd'} eq 'writepr' )
            || ( $query->{'prof'} ne '' )
            || ( $query->{'cmd'} eq 'editprofform' )
            || ( $query->{'cmd'} eq 'editprof' ) );
    ...
    if ( $type eq 'character' || $type eq 'guest' ) {
      ...
      <div class="radio-btn-container">
        <div class="radio-btn-group">
          ...
          <label for="say_type_normal_$type">通常</label>
...
_HTML_
    ...
    if ( CanTsay( $sow, $vil, $curpl ) == 1 ) {
        print <<"_HTML_";
      <span id="submit_monologue_$type"><input name="submit_type" type="submit" value="$sow->{'textrs'}->{'CAPTION_TSAY_PC'}" data-submit-type="tsay"$disabled$net> あと$curpl->{'tsay'}$unit</span>
_HTML_
    ...
    if ( $vil->{'epilogue'} < $vil->{'turn'} ) {
        ...
        if ( $query->{'pno'} >= 0 ) {
            if ( $query->{'ghost'} == 1 ) {
                ...
                print "<a href=\"$sow->{'cfg'}->{'FILE_SOW'}?$linkvalues$postturn$amp" . "ghost=1\">憑</a>\n";
            }
        }
        print "</p>\n\n";
    }
...
sub OutHTMLPageNaviPC {
    ...
    $endpage = $firstpage + $maxrow - 1
      if ( ( $rows->{'end'} != 0 ) || ( !defined( $logs->[$#$logs] ) ) );
    $endpage = -1
      if ( ( $query->{'cmd'} eq 'memo' )
        || ( $query->{'cmd'} eq 'hist' )
        || ( $query->{'rowall'} ne '' ) );
