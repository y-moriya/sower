...
_HTML_
	...
	my $vplcntstart = 16;
	$vplcntstart = $vil->{'vplcntstart'} if ($sow->{'query'}->{'cmd'} eq 'editvilform');
...
_HTML_
	...
	foreach (@$order_roletable) {
		...
		print "      <option value=\"$_\"$selected>$sow->{'textrs'}->{'CAPTION_ROLETABLE'}->{$_}$sow->{'html'}->{'option'}\n";
	}
...
_HTML_
	...
	foreach (@$trsidlist) {
		my $selected = '';
		$selected = " $sow->{'html'}->{'selected'}" if ($vil->{'trsid'} eq $_);
...
_HTML_
	...
	if ($sow->{'query'}->{'cmd'} eq 'editvilform') {
		...
		$checkednoselrole = " $sow->{'html'}->{'checked'}" if ($cfg->{'DEFAULT_NOSELROLE'} > 0);
	}
...
_HTML_
	...
	if ($sow->{'query'}->{'cmd'} eq 'editvilform') {
		$checkedmakersaymenu = " $sow->{'html'}->{'checked'}" if ($vil->{'makersaymenu'} > 0);
	} else {
		$checkedmakersaymenu = " $sow->{'html'}->{'checked'}" if ($cfg->{'DEFAULT_MAKERSAYMENU'} > 0);
	}
...
_HTML_
	...
	if ($sow->{'query'}->{'cmd'} eq 'editvilform') {
		$checkedentrustmode = " $sow->{'html'}->{'checked'}" if ($vil->{'entrustmode'} > 0);
	} else {
		$checkedentrustmode = " $sow->{'html'}->{'checked'}" if ($cfg->{'DEFAULT_ENTRUSTMODE'} > 0);
	}
...
_HTML_
	...
	my $checkedshowall = '';
	if ($sow->{'query'}->{'cmd'} eq 'editvilform') {
		$checkedshowall = " $sow->{'html'}->{'checked'}" if ($vil->{'showall'} > 0);
	} else {
		$checkedshowall = " $sow->{'html'}->{'checked'}" if ($cfg->{'DEFAULT_SHOWALL'} > 0);
	}
...
_HTML_
	...
	if ($sow->{'query'}->{'cmd'} eq 'editvilform') {
		foreach (@$noactlist) {
			my $selected = '';
			$selected = " $sow->{'html'}->{'selected'}" if ($vil->{'noactmode'} eq $i);
		...
		foreach (@$noactlist) {
			my $selected = '';
			$selected = " $sow->{'html'}->{'selected'}" if ($cfg->{'DEFAULT_NOACTMODE'} eq $i);
			print "      <option value=\"$i\"$selected>$_$sow->{'html'}->{'option'}\n";
			$i++;
		}
	}
...
_HTML_
	...
	my $checkednocandy = '';
	if ($sow->{'query'}->{'cmd'} eq 'editvilform') {
		$checkednocandy = " $sow->{'html'}->{'checked'}" if ($vil->{'nocandy'} > 0);
	} else {
		$checkednocandy = " $sow->{'html'}->{'checked'}" if ($cfg->{'DEFAULT_NOCANDY'} > 0);
	}
...
_HTML_
	...
	my $checkednofreeact = '';
	if ($sow->{'query'}->{'cmd'} eq 'editvilform') {
		$checkednofreeact = " $sow->{'html'}->{'checked'}" if ($vil->{'nofreeact'} > 0);
	} else {
		$checkednofreeact = " $sow->{'html'}->{'checked'}" if ($cfg->{'DEFAULT_NOFREEACT'} > 0);
	}
...
_HTML_
	...
	my $checkedtimestamp = '';
	if ($sow->{'query'}->{'cmd'} eq 'editvilform') {
		$checkedtimestamp = " $sow->{'html'}->{'checked'}" if ($vil->{'timestamp'} > 0);
	} else {
		$checkedtimestamp = " $sow->{'html'}->{'checked'}" if ($cfg->{'DEFAULT_TIMESTAMP'} > 0);
	}
