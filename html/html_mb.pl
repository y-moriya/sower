...
sub OutHTMLHeaderMb {
    ...
    print <<"_HTML_";
<head>
  <title>$title$sow->{'cfg'}->{'NAME_SW'}</title>
</head>
...
sub OutHTMLTurnNaviMb {
    ...
    # ページリンク表示
    &OutHTMLPageNaviMb( $sow, $vil, $logs, $list, $rows )
      if ( ( defined($list) ) && ( $sow->{'turn'} <= $vil->{'epilogue'} ) );
    ...
    my $first = '頭';
    $first = "<a href=\"$urlfirst$amp" . "move=first\" accesskey=\"1\">頭</a>"
      if ( $rows->{'start'} == 0 );
    my $last = '末';
    $last = "<a href=\"$urlfirst$amp" . "move=last\" accesskey=\"9\">末</a>"
      if ( $rows->{'end'} == 0 );
    my $wrformmbt = '書';
    my $wrformmbb = '書';
    my $cmdwrite  = 'c=wrformmb';
    $cmdwrite = "c=wrmemoformmb$amp" . "cmdfrom=$query->{'cmd'}"
      if ( ( $query->{'cmd'} eq 'memo' ) || ( $query->{'cmd'} eq 'hist' ) );
...
sub GetPrevLink {
    ...
    if ( ( $rows->{'start'} == 0 ) && ( defined( $logs->[0]->{'mestype'} ) ) ) {
        my $startlog = $logs->[0];
        $startlog->{'mestype'} = $sow->{'MESTYPE_SAY'}
          if ( !defined( $startlog->{'mestype'} ) );
        if (
               ( $vil->isepilogue() == 0 )
            && ( defined( $startlog->{'maskedid'} ) )
            && (   ( $startlog->{'mestype'} == $sow->{'MESTYPE_INFOSP'} )
...
sub GetNextLink {
    ...
    if (   ( $rows->{'end'} == 0 )
        && ( @$logs > 0 )
        && ( defined( $logs->[$#$logs]->{'mestype'} ) ) )
    {
        my $endlog = $logs->[$#$logs];
        $endlog->{'mestype'} = $sow->{'MESTYPE_SAY'}
          if ( !defined( $endlog->{'mestype'} ) );
        if (
               ( $vil->isepilogue() == 0 )
            && ( defined( $endlog->{'maskedid'} ) )
            && (   ( $endlog->{'mestype'} == $sow->{'MESTYPE_INFOSP'} )
