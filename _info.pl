...
2014/06/06
<ul>
<li><a href="http://web1.kcn.jp/so-u-ko-u/">妖怪のたおしかた／おくみつ</a>様のキャラセット＆文章系「仰げば狼」を実装しました。
<li>一部を除くほとんどの発言制限において、進行中の表\発言のポイント以外のポイント数（独り言・囁き等）を10倍にしました。
</ul>
2014/04/23
<ul>
<li>同一日24時間経過による発言数回復を削除しました。（日が変われば今まで通り回復します）</li>
</ul>
_HTML_
