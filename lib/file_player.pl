...
sub gettargetlist {
	...
	foreach $livepl (@$livepllist) {
		...
		next if (($self->{'role'} == $sow->{'ROLEID_TRICKSTER'}) && ($livepl->{'uid'} eq $sow->{'cfg'}->{'USERID_NPC'})); # ピクシーの対象にはダミーキャラを含まない
...
#			next if ($bond > 0); # 運命の絆で結ばれている相手でも除外しない
		...
		if (($self->iswolf() > 0) && ($cmd ne 'vote')) {
			...
			next if (($vil->{'turn'} == 1) && ($livepl->{'uid'} ne $sow->{'cfg'}->{'USERID_NPC'})); # １日目の襲撃対象はダミーキャラのみ
		}
...
sub gettargetlistwithrandom {
	...
	# おまかせ（襲撃対象のみ）
	if (($cmd ne 'vote') && ($self->iswolf() > 0) && ($vil->{'turn'} > 1)) {
		my %target = (
	...
	$randomtarget = 0 if (($cmd ne 'vote') && ($self->iswolf() > 0) && ($vil->{'turn'} == 1)); # １日目の襲撃対象にはランダムを含まない
	if ($randomtarget > 0) {
		my %randomtarget = (
			chrname => 'ランダム',
...
sub iswin {
	...
	my $win = 2; # 負け
	$win = 1 if ($vil->{'winner'} == $sow->{'ROLECAMP'}[$self->{'role'}]); # 勝ち
