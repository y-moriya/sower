...
sub SetDataCmdWriteMemo {
	...
	$debug->raise($sow->{'APLOG_NOTICE'}, "あなたは死んでいます。","you dead.[$errfrom]") if (($writepl->{'live'} ne 'live') && ($vil->isepilogue() == 0));

	# 残りアクションがゼロの時
	require "$sow->{'cfg'}->{'DIR_LIB'}/write.pl";
	my ($dummy, $saytype) = &SWWrite::GetMesType($sow, $vil, $writepl);
	$debug->raise($sow->{'APLOG_NOTICE'}, "アクションが足りません。","not enough saypoint.[$saytype: $writepl->{$saytype} / 1]") if ($writepl->{$saytype} <= 0);
