...
sub CheckValidityMakeVil {
    ...
    $sow->{'debug'}
      ->raise( $sow->{'APLOG_NOTICE'}, "ログインして下さい。", "no login.$errfrom" )
      if ( $sow->{'user'}->logined() <= 0 );
    $sow->{'debug'}
      ->raise( $sow->{'APLOG_CAUTION'}, "村の作成はできません。", "cannot vmake.$errfrom" )
    ...
    $sow->{'debug'}
      ->raise( $sow->{'APLOG_NOTICE'}, '文字列リソースが未選択です。', "no trsid.$errfrom" )
      if ( $query->{'trsid'} eq '' );
    $sow->{'debug'}
      ->raise( $sow->{'APLOG_CAUTION'}, '登場人物が未選択です。', "no csid.$errfrom" )
      if ( $query->{'csid'} eq '' );    # 通常起きない
    $sow->{'debug'}
      ->raise( $sow->{'APLOG_CAUTION'}, '役職配分が未選択です。', "no roletable.$errfrom" )
      if ( $query->{'roletable'} eq '' );    # 通常起きない
    $sow->{'debug'}
      ->raise( $sow->{'APLOG_CAUTION'}, '投票方法が未選択です。', "no votetype.$errfrom" )
      if ( $query->{'votetype'} eq '' );     # 通常起きない
    $sow->{'debug'}
      ->raise( $sow->{'APLOG_CAUTION'}, '更新時間（時）が未選択です。', "no hour.$errfrom" )
      if ( !defined( $query->{'hour'} ) );    # 通常起きない
    $sow->{'debug'}->raise( $sow->{'APLOG_CAUTION'},
        '更新時間（時）は0～23の範囲で選んで下さい。', "invalid hour.$errfrom" )
    ...
    $sow->{'debug'}->raise( $sow->{'APLOG_CAUTION'},
        '更新時間（分）は0～59の範囲で選んで下さい。', "invalid minite.$errfrom" )
      if ( ( $query->{'minite'} < 0 ) || ( $query->{'minite'} > 59 ) ); # 通常起きない
    $sow->{'debug'}->raise( $sow->{'APLOG_CAUTION'},
        '更新間隔が未選択です。', "no updinterval.$errfrom" )
      if ( !defined( $query->{'updinterval'} ) );                       # 通常起きない
    $sow->{'debug'}->raise( $sow->{'APLOG_CAUTION'},
        '更新間隔は1～3の範囲で選んで下さい。', "invalid updinterval.$errfrom" )
      if ( ( $query->{'updinterval'} < 1 ) || ( $query->{'updinterval'} > 3 ) )
      ;                                                                 # 通常起きない
    $sow->{'debug'}
      ->raise( $sow->{'APLOG_NOTICE'}, '定員が未入力です。', "no vplcnt.$errfrom" )
...
"定員は$sow->{'cfg'}->{'MINSIZE_VPLCNT'}～$sow->{'cfg'}->{'MAXSIZE_VPLCNT'}の範囲で入力して下さい。",
    ...
    if ( $query->{'starttype'} eq 'wbbs' ) {
        $sow->{'debug'}->raise( $sow->{'APLOG_NOTICE'},
            '最低人数が未入力です。', "no vplcntstart.$errfrom" )
...
"最低人数は$sow->{'cfg'}->{'MINSIZE_VPLCNT'}～$sow->{'cfg'}->{'MAXSIZE_VPLCNT'}の範囲で入力して下さい。",
        ...
        $sow->{'debug'}->raise( $sow->{'APLOG_NOTICE'},
            '最低人数は定員以下の人数で入力して下さい。', "too many vplcntstart.$errfrom" )
          if ( $query->{'vplcntstart'} > $query->{'vplcnt'} );
    }

    $sow->{'debug'}->raise( $sow->{'APLOG_CAUTION'},
        '発言制限が未選択です。', "no saycnttype.$errfrom" )
      if ( $query->{'saycnttype'} eq '' );    # 通常起きない
    $sow->{'debug'}->raise( $sow->{'APLOG_CAUTION'},
        '非対応の参加制限を選択しています。', "invalid entrylimit.$errfrom" )
    ...
    if ( $query->{'roletable'} eq 'custom' ) {
        $sow->{'debug'}
          ->raise( $sow->{'APLOG_NOTICE'}, '人狼がいません。', "no wolves. $errfrom" )
          if ( $wolves <= 0 );
...
"invalid vplcnt or total plcnt.[$query->{'vplcnt'} != $total] $errfrom"
        ...
        if ( $wolves >= ( $total - $total_cpossess - $total_hamster - 1 ) / 2 )
        {
            if ( $total_cpossess > 0 ) {
                ...
                $sow->{'debug'}->raise( $sow->{'APLOG_NOTICE'},
                    '人狼が多すぎます。', "too many wolves. $errfrom" );
            }
        }
        $sow->{'debug'}->raise(
            $sow->{'APLOG_NOTICE'},
