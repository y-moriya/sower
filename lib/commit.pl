...
sub StartSession {
	...
	$logfile->writeinfo('', $sow->{'MESTYPE_INFONOM'}, $sow->{'textrs'}->{'NOSELROLE'}) if ($vil->{'noselrole'} > 0);
...
sub UpdateSession {
	...
	if ($vil->{'epilogue'} < $vil->{'turn'}) {
		...
		if ($scrapvil == 0) {
			# 突然死
			my ($history, $suddendeathpl) = &SuddenDeath($sow, $vil, $logfile) if ($sow->{'cfg'}->{'ENABLED_SUDDENDEATH'} > 0);
			...
			if ($sow->{'cfg'}->{'ENABLED_SCORE'} > 0) {
				...
				if (defined($score->{'file'})) {
					...
					$score->writeupdate($vil->{'turn'}, $suddendeathpl, $executepl, $seertargetpl, $guardtargetpl, \@killtargetpl);
				}
				$score->close();
			}
		...
		if (($winner > 0) || ($scrapvil > 0)) { # ゲーム終了
			...
			my $livesnametext = join($sow->{'textrs'}->{'ANNOUNCE_LIVES'}->[1], @livesnamelist);
...
sub GetResultSeer {
	...
	$result = $sow->{'textrs'}->{'RESULT_SEER'}->[2] if ($targetpl->iswolf() > 0); # 人狼/呪狼/智狼
		...
		if ($ability ne '') {
			if ($srcpl->{'target'} == $sow->{'TARGETID_RANDOM'}) {
				...
				if ($srcpl->{'role'} == $sow->{'ROLEID_TRICKSTER'}) {
					$srctargetpno = $srcpl->{'target2'} if (($srcpl->{'target2'} >= 0) && ($srcpl->{'live'} eq 'live'));
				}
				&SetRandomTargetSingle($sow, $vil, $srcpl, 'target', $logfile, $srctargetpno);
			}

			if (($srcpl->{'role'} == $sow->{'ROLEID_TRICKSTER'}) && ($srcpl->{'target2'} == $sow->{'TARGETID_RANDOM'})) {
...
sub SetRandomTargetSingle {
	...
	if (@$targetlist == 0) {
		# 対象候補が存在しない
		$plsingle->{$targetid} = -1;
		$logfile->writeinfo($plsingle->{'uid'}, $sow->{'MESTYPE_INFOSP'}, $plsingle->getchrname() . "の対象($targetid)候補がありません。");
		return;
	}

	$plsingle->{$targetid} = $targetlist->[int(rand(@$targetlist))]->{'pno'};
...
sub SetBondsTarget {
	...
	foreach $plsingle (@$livepllist) {
		...
		if ($targetpl->{'live'} ne 'live') {
			# 設定対象１が突然死している時
			my $srctargetpno;
			$srctargetpno = $plsingle->{'target2'} if (($plsingle->{'target2'} >= 0) && ($target2pl->{'live'} eq 'live'));
...
sub Execution {
	...
	foreach (@$livepllist) {
		$_->{'randomentrust'} = '';
		if (($_->{'entrust'} > 0) && ($_->{'vote'} == $sow->{'TARGETID_RANDOM'})) {
			$_->{'vote'} = $livepllist->[rand(@$livepllist - 1)]->{'pno'};
			$_->{'vote'} = $livepllist->[$#$livepllist]->{'pno'} if ($_->{'vote'} == $_->{'pno'});
	...
	foreach $curpl (@$livepllist) {
		...
		while ($srcpl->{'entrust'} > 0) {
			...
			if (($i > @$livepllist) || ($srcpl->{'live'} ne 'live')) {
				...
				foreach (@entrusts) {
					...
					# 投票先をランダムに設定
					my $entrusttext = $sow->{'textrs'}->{'ANNOUNCE_ENTRUST'}->[1];
					my $chrname = $_->getchrname();
		...
		if (@entrusts > 0) {
			...
			for ($i = 0; $i < @entrusts; $i++) {
				...
				# 委任メッセージ表示
				my $entrusttext = $sow->{'textrs'}->{'ANNOUNCE_ENTRUST'}->[$randomvote];
	...
	foreach (@$livepllist) {
		...
		$logfile->writeinfo($_->{'uid'}, $sow->{'MESTYPE_INFOSP'}, $votetext) if ($vil->{'votetype'} ne 'sign');
	}
...
sub Seer {
	...
	foreach (@$livepllist) {
		if ($_->{'role'} eq $sow->{'ROLEID_SEER'}) {
			...
			# ハム・コウモリ・ピクシー呪殺
			if (($targetpl->{'live'} eq 'live') && ($targetpl->ishamster() > 0)) {
				push(@hampl, $targetpl);
			...
			# 呪狼
			if (($targetpl->{'live'} eq 'live') && ($targetpl->{'role'} == $sow->{'ROLEID_CWOLF'})) {
				push(@hampl, $_);
				$_->{'live'} = 'cursed';
				$_->{'deathday'} = $vil->{'turn'};
				my $user = SWUser->new($sow);
...
sub SelectKill {
	...
	foreach (@$livepllist) {
		...
		if ($targetpl->iswolf() > 0) {
			...
			$sow->{'debug'}->writeaplog($sow->{'APLOG_WARNING'}, "target is a wolf.[wolfid=$_->{'uid'}, target=$targetpl->{'uid'}]");
			next;
		}
		...
		if ($vil->{'turn'} > 2) {
			...
			foreach (@$livepllist) {
				...
				if ($_->{'target'} == $targetpl->{'pno'}) {
					...
					$_->{'history'} .= "$result_guard<br>" if (defined($targetpl->{'uid'}));
					$deadflag = 'live';
				}
			}
		}
		...
		if ($deadflag ne 'live') {
			...
			foreach (@$livepllist) {
				if ($_->iswolf() > 0) {
					...
					$result_kill= $sow->{'textrs'}->{'RESULT_KILLIW'} if ($_->{'role'} == $sow->{'ROLEID_INTWOLF'});
...
sub SetInitVoteTarget {
	...
	foreach (@$livepllist) {
		...
		&SetInitVoteTargetSingle($sow, $vil, $_, 'target2', $logfile, $_->{'target'}) if ($_->{'role'} == $sow->{'ROLEID_TRICKSTER'});

		if ($_->iswolf() > 0) {
			if ($vil->{'turn'} != 1) {
				$_->{'target'} = $sow->{'TARGETID_TRUST'}; # 人狼はおまかせ
				$sow->{'debug'}->writeaplog($sow->{'APLOG_OTHERS'}, "ChangeTarget UNDEF (Wolf): $_->{'uid'}($_->{'pno'})=$_->{'target'}");
			}
		}
	}
...
#	my $listtext = "★" . $plsingle->getchrname() . "の初期対象($targetid)候補★<br>";
	if (@$targetlist > 0) {
		$plsingle->{$targetid} = $targetlist->[int(rand(@$targetlist))]->{'pno'};
		$sow->{'debug'}->writeaplog($sow->{'APLOG_OTHERS'}, "SetInitVote/Target[$targetid]: $plsingle->{'uid'}($plsingle->{'pno'})=randtarget");
