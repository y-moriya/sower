...
sub executesay {
	...
	if ((($say->{'mestype'} == $sow->{'MESTYPE_INFOSP'}) || ($say->{'mestype'} == $sow->{'MESTYPE_TSAY'})) && ($self->{'vil'}->isepilogue() == 0)) {
		$cntmaskedid = "$sow->{'LOGCOUNT_MESTYPE'}->[$say->{'mestype'}]$sow->{'LOGCOUNT_SUBID'}->{$sow->{'LOGSUBID_SAY'}}";
	...
	$logcntid = "$sow->{'LOGCOUNT_MESTYPE'}->[$sow->{'MESTYPE_UNDEF'}]$sow->{'LOGCOUNT_SUBID'}->{$say->{'logsubid'}}" if ($say->{'undef'} > 0); # 襲撃メッセージ
...
sub fixque {
	...
	foreach (@$quelist) {
		...
		if (($log->{'mestype'} != $sow->{'MESTYPE_QUE'}) || ($logcount ne $_->{'queid'})) {
			# ログデータに該当するログがない。
			...
			$log->{'logid'} = &SWLog::CreateLogID($sow, $sow->{'MESTYPE_SAY'}, $log->{'logsubid'}, $logcnt->{'countsay'});
			$logcnt->{'countsay'}++;
...
sub delete {
	...
	if ($queindexno >= 0) {
		...
		my ($logmestype, $logsubid, $logcount) = &SWLog::GetLogIDArray($log);
		if (($log->{'mestype'} != $sow->{'MESTYPE_QUE'}) || ($logcount ne $del_queid)) {
			# ログデータに該当するログがない（本来ありえない）
			$sow->{'debug'}->writeaplog($sow->{'APLOG_WARNING'}, "Cancel, [queid=$del_queid, logid=$log->{'logid'}]");
		} else {
			$sow->{'debug'}->raise($sow->{'APLOG_CAUTION'}, "削除しようとした保留中の発言が見つかりません。", "cannot delete say.[cmd=cancel, vid=$self->{'vil'}->{'vid'}, del_queid=$del_queid]") if ($log->{'uid'} ne $sow->{'uid'}); # 権限のないユーザからの撤回コマンドが飛んできた場合、セキュリティのためこういう警告にする
...
sub CheckLogPermition {
	...
	} elsif (($self->{'vil'}->isepilogue() > 0) && ($log->{'mestype'} != $sow->{'MESTYPE_QUE'})) {
		# エピローグ中
		$logpermit = 1;
	} elsif (($logined > 0) && (defined($curpl->{'uid'}))) {
		# 進行中
		$logpermit = 1 if (($log->{'uid'} eq $curpl->{'uid'}) && ($log->{'mestype'} != $sow->{'MESTYPE_WSAY'}) && ($log->{'mestype'} != $sow->{'MESTYPE_SPSAY'}) && ($log->{'mestype'} != $sow->{'MESTYPE_BSAY'})); # 自分の発言
		$logpermit = 0 if (($query->{'mode'} eq 'whisper') && ($curpl->iswhisper() > 0)); # 囁きのみモード

		if (($curpl->{'live'} eq 'live') || ($sow->{'cfg'}->{'ENABLED_PERMIT_DEAD'} > 0)) {
			# ささやき
			$logpermit = 1 if ((($curpl->iswolf() > 0) || ($curpl->{'role'} == $sow->{'ROLEID_CPOSSESS'})) && ($log->{'mestype'} == $sow->{'MESTYPE_WSAY'}) && ($query->{'mode'} ne 'human'));
			# 共鳴
			$logpermit = 1 if (($curpl->{'role'} == $sow->{'ROLEID_SYMPATHY'}) && ($log->{'mestype'} == $sow->{'MESTYPE_SPSAY'}) && ($query->{'mode'} ne 'human'));
			# 念話
			$logpermit = 1 if (($curpl->{'role'} == $sow->{'ROLEID_WEREBAT'}) && ($log->{'mestype'} == $sow->{'MESTYPE_BSAY'}) && ($query->{'mode'} ne 'human'));
		}
		if ($curpl->{'live'} ne 'live') {
			$logpermit = 1 if ($log->{'mestype'} == $sow->{'MESTYPE_GSAY'}); # うめき
			$logpermit = 1 if (($self->{'vil'}->{'showall'} > 0) && ($log->{'mestype'} >= $sow->{'MESTYPE_WSAY'})); # 墓下公開
		}
  ...
  if (defined($query->{'pno'})) {
      ...
      if ($query->{'pno'} == '-2'){
        $logpermit = 0 if ($log->{'mestype'} ne $sow->{'MESTYPE_TSAY'});
      } elsif ($query->{'pno'} == '-3'){
        $logpermit = 0 if ($log->{'mestype'} ne $sow->{'MESTYPE_WSAY'});
      } elsif ($query->{'pno'} == '-4'){
        $logpermit = 0 if ($log->{'mestype'} ne $sow->{'MESTYPE_GSAY'});
      } elsif ($query->{'pno'} == '-5'){
        $logpermit = 0 if ($log->{'mestype'} ne $sow->{'MESTYPE_MAKER'});
      } elsif ($query->{'pno'} == '-6'){
