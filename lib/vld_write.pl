...
sub CheckValidityWrite {
    ...
    $debug->raise(
        $sow->{'APLOG_NOTICE'},
        "発言が短すぎます（$lenmes バイト）。$sow->{'cfg'}->{'MINSIZE_MES'} バイト以上必要です。",
    ...
    $debug->raise( $sow->{'APLOG_CAUTION'},
        "発言種別が不正です。", "invalid mestype.$errfrom" )
      if ( $cntenabled > 1 );    # 通常起きない
            ...
            if ( $query->{'guest'} ne '' ) {
                if ( $query->{'writepwd'} ne $vil->{'entrypwd'} ) {
                    $debug->raise( $sow->{'APLOG_NOTICE'},
                        'パスワードが違います。', "invalid writepwd.$errfrom" );
                }
            }
        }
    }
    ...
    if ( ( $vil->{'guestmenu'} > 0 ) && ( $query->{'guest'} ne '' ) ) {
        $debug->raise( $sow->{'APLOG_CAUTION'},
            "傍観者発言は無効です。", "invalid guest.$errfrom" );
    }
