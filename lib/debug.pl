...
sub writeaplog {
    ...
    $datafile = $sow->{'cfg'}->{'FILE_APLOG'}
      if ( ( $type eq $sow->{'APLOG_POSTED'} )
        || ( $type eq $sow->{'APLOG_OTHERS'} ) );
    $datafile = "$sow->{'cfg'}->{'DIR_LOG'}/$sow->{'cfg'}->{'FILE_304LOG'}"
      if ( ( $type eq 'http' )
        && ( defined( $sow->{'cfg'}->{'FILE_304LOG'} ) ) );    # デバッグ用
...
#	die "[$type] $mes [from $pkg $fname line $line]" if ($sow->{'lock'}->{'lock'} ne 'lock');
    ...
    # 書き込み
    open( APLOG, ">>$datafile" )
      || die "aplog could not create.[from $pkg $fname line $line]";
    my $t = $sow->{'dt'}->cvtdt( $sow->{'time'} );
    print APLOG "($type) [$t] $mes";

    # WARNING と CAUTION の場合はエラーレベルに関係なく cookie と 入力値を出力
    if (   ( $type eq $sow->{'APLOG_WARNING'} )
        || ( $type eq $sow->{'APLOG_CAUTION'} ) )
    {
...
sub OutHTMLError {
    ...
    require "$sow->{'cfg'}->{'DIR_LIB'}/http.pl";
    $sow->{'html'} = SWHtml->new($sow)
      if ( !defined( $sow->{'html'}->{'sow'} ) );
    $sow->{'http'} = SWHttp->new($sow)
      if ( !defined( $sow->{'http'}->{'sow'} ) );
    $sow->{'http'}->outheader();
...
<script type="text/javascript">
  document.write('<p class="return">');
  document.write('<a href="javascript:history.back();">戻る</a>');
  document.write('<\/p>');
</script>
<noscript>
<p class="return">
<a href="$cfg->{'BASEDIR_CGI'}/$cfg->{'FILE_SOW'}">トップページに戻る</a>
</p>
</noscript>
