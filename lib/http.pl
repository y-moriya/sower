...
sub outheader {
    ...
    if ( $self->{'notmodified'} > 0 ) {
        ...
        $sow->{'debug'}->writeaplog( 'http', "Status: 304 Not Modified" )
          if ( defined( $sow->{'cfg'}->{'FILE_304LOG'} ) );
        return 0;    # ヘッダ出力のみ
    }
    ...
    elsif (( $self->{'contenttype'} eq 'xhtml' )
        && ( index( $ENV{'HTTP_ACCEPT'}, 'application/xhtml+xml' ) >= 0 ) )
    {
    ...
    if ( $self->{'location'} ne '' ) {
        ...
        if ( ( $operaver >= 0 ) && ( $operaver < 9 ) ) {
            ...
            $self->{'location'} = substr( $self->{'location'}, 0, $pos )
              if ( $pos >= 0 );
        }
        ...
        $sow->{'debug'}->writeaplog( 'http', "Status: 303 See Other" )
          if ( defined( $sow->{'cfg'}->{'FILE_304LOG'} ) );
...
_HTML_
    ...
    if ( $self->{'unavailable'} > 0 ) {
        ...
        $sow->{'debug'}->writeaplog( 'http', "Status: 503 Service Unavailable" )
          if ( defined( $sow->{'cfg'}->{'FILE_304LOG'} ) );
    }
    else {
        $header = "Status: 200 OK\n" . $header;
        $sow->{'debug'}->writeaplog( 'http', "Status: 200 OK" )
          if ( defined( $sow->{'cfg'}->{'FILE_304LOG'} ) );
    }
    ...
    # gzip転送をしない
    if (   !( -e $sow->{'cfg'}->{'FILE_GZIP'} )
        || !$encoding
        || ( $self->{'modperl'} != 0 )
        || !open( GZIP, "| " . $sow->{'cfg'}->{'FILE_GZIP'} ) )
    {
        $self->CallSendHTTPHeader($header);
        return 1;
    }
