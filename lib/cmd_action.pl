...
sub SetDataCmdAction {
	...
	if (defined($query->{'target'}) && ($query->{'target'} >= 0)) {
		if ((!defined($query->{'actionno'})) || ($query->{'actionno'} != -2)) {
			...
			$debug->raise($sow->{'APLOG_CAUTION'}, "アクション対象の人は死んでいます。","target is dead.") if (($targetpl->{'live'} ne 'live') && ($vil->isepilogue() == 0));
  ...
  $selectact = 'freetext' if (($query->{'actionno'} == -3) || ((defined($query->{'selectact'})) && ($query->{'selectact'} eq 'freetext')));
	...
	if ($selectact eq 'template') {
		...
		if ($query->{'actionno'} != -2) {
			...
			} elsif ((!defined($targetpl->{'pno'})) && ($actions->[$query->{'actionno'}] =~ /^[をにがのへ]/)) { # 文頭が格助詞で始まるものは対象が必要。
				$sow->{'debug'}->raise($sow->{'APLOG_NOTICE'}, "アクションの対象が未選択です。", "no target.$errfrom");
			} elsif (defined($targetpl->{'pno'}) && !($actions->[$query->{'actionno'}] =~ /^[をにがのへ]/)) {
				$mes = '';
			}
		}
		...
		if ($query->{'actionno'} == -1) {
			...
			$debug->raise($sow->{'APLOG_CAUTION'}, "促し無しオプションが有効です。", "nocandy option.") if ($vil->{'nocandy'} > 0);
		...
		# 自由入力アクション
		$debug->raise($sow->{'APLOG_CAUTION'}, "自由入力アクション無しオプションが有効です。", "nofreeact option.") if ($vil->{'nofreeact'} > 0);
