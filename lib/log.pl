...
sub GetAnchorlogID {
    ...
    my $loganchor     = '';
    $loganchor = "$loganchormark->{$logmestype}$logcntnum"
      if ( defined( $loganchormark->{$logmestype} ) );

    $loganchor = '' if ( $logcntnum == $sow->{'LOGCOUNT_UNDEF'} );
    $loganchor = ''
      if ( ( $log->{'mestype'} == $sow->{'MESTYPE_TSAY'} )
        && ( $vil->isepilogue() == 0 ) );
...
sub ReplaceAnchorHTML {
    ...
    $$mes =~
s/(s?https?:\/\/[^\/<>\s]+)[-_.!~*'()a-zA-Z0-9;\/?:\@&=+\$,%#]+/<a href=\"$&\" class=\"res_anchor\"$blank>&lt;$1&gt;<\/a>/g;
...
sub GetPopupAnchor {
    ...
    my $logidxno =
      $anchor->{'logfile'}->{'logindex'}->{'file'}->getbyid($logid);
    if ( $logidxno >= 0 ) {
        my $logidx =
          $anchor->{'logfile'}->{'logindex'}->{'file'}->getlist->[$logidxno];
...
sub CvtRandomText {
    ...
    $mes =~
s/\[\[$cfg->{'RANDOMTEXT_LIVES'}\]\]/{my $a = int(rand(scalar(@$livepllist))); sprintf("<strong>&lt;&lt;%s&gt;&gt;<\/strong>", $livepllist->[$a]->getchrname())}/eg;

    # omikuji
    $mes =~
s/\[\[$cfg->{'RANDOMTEXT_MIKUJI'}\]\]/{my $a = int(rand(6))+1; my $b = int(rand(6))+1; my $c = int(rand(6))+1; sprintf("<strong>*%s*<\/strong>", $cfg->{'MIKUJI'}->[$a+$b+$c - 3])}/eg;

    # role
    my $rolename = $sow->{'textrs'}->{'ROLENAME'};
    $mes =~
s/\[\[$cfg->{'RANDOMTEXT_ROLE'}\]\]/{my $a = int(rand(scalar(@$rolename)-1))+1; sprintf("<strong>((%s))<\/strong>", $rolename->[$a])}/eg;
