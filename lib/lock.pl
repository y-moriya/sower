...
sub gflock {
	...
	$sow->{'debug'}->raise($sow->{'APLOG_WARNING'}, 'ファイルロックに失敗しました。', 'lockfile not found.') if !(-e $sow->{'cfg'}->{'FILE_LOCK'}); # ファイルがない
	open (LOCK, "+<$sow->{'cfg'}->{'FILE_LOCK'}") || $sow->{'debug'}->raise($sow->{'APLOG_WARNING'}, 'ファイルロックに失敗しました。', 'lockfile could not open.');
	eval { flock(LOCK, 2); };
	$self->{'lock'} = 'lock';
