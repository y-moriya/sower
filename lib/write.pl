...
sub ExecuteCmdWrite {
	...
	if ($writepl->{'emulated'} == 0) {
		...
		# 発言数がない（仮）
		$sow->{'debug'}->raise($sow->{'APLOG_NOTICE'}, "発言数が足りません。","not enough saypoint.[$saytype: $writepl->{$saytype} / $saypoint]") if ((!defined($writepl->{$saytype})) || (($writepl->{$saytype} - $saypoint) < 0));
	}
	...
	if ($writepl->{'emulated'} == 0) {
		if (($act == 0) && ($writepl->{'lastwritepos'} >= 0)) {
			...
			$checkmestype = $sow->{'MESTYPE_SAY'} if ($mestype == $sow->{'MESTYPE_QUE'});
			my $checklogmestype = $log->{'mestype'};
			$checklogmestype = $sow->{'MESTYPE_SAY'} if ($log->{'mestype'} == $sow->{'MESTYPE_QUE'});
			$sow->{'debug'}->raise($sow->{'APLOG_NOTICE'}, "直前の発言と同じ内容の発言をしようとしています。","same last mes.") if (($logmes eq $mes) && ($checklogmestype == $checkmestype));
		}
	}

	my $logsubid = $sow->{'LOGSUBID_SAY'};
	$logsubid = $sow->{'LOGSUBID_ACTION'} if ($act > 0);
	$logsubid = $sow->{'LOGSUBID_BOOKMARK'} if ((defined($query->{'actionno'})) && ($query->{'actionno'} == -2));
	...
	if ($writepl->{'emulated'} == 0) {
		...
		if ($vil->isepilogue() > 0) {
			...
			if (($act == 0) && (($mestype == $sow->{'MESTYPE_QUE'}) || ($mestype == $sow->{'MESTYPE_GSAY'}))) {
				# 発言済み回数／pt
				$writepl->{'saidcount'}++ if ($mestype == $sow->{'MESTYPE_GSAY'});
				$writepl->{'saidpoint'} += $saypoint;
			}
		}
...
sub GetMesType {
	...
	if (($vil->isepilogue() > 0) && (($query->{'think'} ne 'on') || ($cfg->{'ENABLED_TSAY_EP'} == 0))) {
		# エピローグ発言
		$mestype = $sow->{'MESTYPE_QUE'};
		$mestype = $sow->{'MESTYPE_SAY'} if ($cfg->{'ENABLED_DELETED'} > 0);
		$saytype = 'esay';
	} elsif (($vil->{'turn'} == 0) && (($cfg->{'ENABLED_TSAY_PRO'} == 0) || ($query->{'think'} ne 'on'))) {
		# プロローグ発言
		$mestype = $sow->{'MESTYPE_QUE'};
		$saytype = 'psay';
	} elsif (($query->{'think'} eq 'on') && (($writepl->{'live'} eq 'live') || ($cfg->{'ENABLED_TSAY_GRAVE'} > 0) || ($cfg->{'ENABLED_TSAY_GUEST'} > 0))) {
		# 独り言
		...
		$saytype = 'gsay';
	} elsif ((($writepl->iswolf() > 0) || ($writepl->{'role'} == $sow->{'ROLEID_CPOSSESS'})) && ($query->{'wolf'} eq 'on')) {
		# ささやき
		$mestype = $sow->{'MESTYPE_WSAY'};
		$saytype = 'wsay';
	} elsif (($writepl->{'role'} == $sow->{'ROLEID_SYMPATHY'}) && ($query->{'sympathy'} eq 'on')) {
		# 共鳴
		$mestype = $sow->{'MESTYPE_SPSAY'};
		$saytype = 'spsay';
	} elsif (($writepl->{'role'} == $sow->{'ROLEID_WEREBAT'}) && ($query->{'werebat'} eq 'on')) {
		# 念話
	...
	if (($query->{'cmd'} eq 'action') || ($query->{'cmd'} eq 'wrmemo')) {
		# アクション
		$saytype = 'say' if (($saytype eq 'psay') || ($saytype eq 'esay'));
		$saytype .= '_act';
		$mestype = $sow->{'MESTYPE_SAY'} if ($mestype == $sow->{'MESTYPE_QUE'});
	}

	if (($query->{'admin'} ne '') && ($sow->{'uid'} eq $cfg->{'USERID_ADMIN'})) {
		# 管理人発言
