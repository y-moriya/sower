...
<ul>
  ...
  <li>最新日のメモ・メモ履歴を表\示している時は、ログへのリンクに #newinfo を加えるようにしました。</li>
