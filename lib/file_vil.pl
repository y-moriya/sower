...
sub getinfocap {
	...
	if ($infocap eq 'lastcnt') {
	...
	} elsif ($infocap eq 'updatedt' || $infocap eq 'updhour' || $infocap eq 'updminite') {
		$resultcap = sprintf("%02d時%02d分", $self->{'updhour'}, $self->{'updminite'});
	} elsif ($infocap eq 'updinterval') {
		$resultcap = sprintf('%02d時間', $self->{'updinterval'} * 24);
	} elsif ($infocap eq 'roletable') {
		$resultcap = $sow->{'textrs'}->{'CAPTION_ROLETABLE'}->{$self->{'roletable'}};
		...
		$resultcap = $sow->{'cfg'}->{'COUNTS_SAY'}->{$self->{'saycnttype'}}->{'CAPTION'};

	} elsif ($infocap eq 'starttype') {
		$resultcap = $sow->{'basictrs'}->{'STARTTYPE'}->{$self->{'starttype'}};
		...
		$resultcap = $sow->{'cfg'}->{'RATING'}->{$self->{'rating'}}->{'CAPTION'};

	} elsif ($infocap eq 'noactmode') {my $noactlist = $sow->{'cfg'}->{'NOACTLIST'};
