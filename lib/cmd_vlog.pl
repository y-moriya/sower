...
sub CheckUpdateSession {
	...
	$isupdate = 2 if (($sow->{'time'} >= $vil->{'nextcommitdt'}) && ($cntlivepl == $commitedpl) && ($cntlivepl > 0)); # 全員が時間を進めている
...
#			$vil->{'nextchargedt'} = $sow->{'dt'}->getnextupdatedt($vil, $sow->{'time'}, 1, 0);
	...
	if ($vil->{'turn'} == 0) {
		if (($vil->{'starttype'} eq 'wbbs') && (@$pllist >= $vil->{'vplcntstart'})) {
			# 村開始（人狼BBS型）
...
sub SetHTTPUpdateVLog {
	...
	if (defined($curpl)) {
		...
		if (($curpl->{'live'} eq 'live') || ($sow->{'cfg'}->{'ENABLED_PERMIT_DEAD'} > 0)) {
			# ささやき
			$modified = $vil->{'modifiedwsay'} if ((($curpl->iswolf() > 0) || ($curpl->{'role'} == $sow->{'ROLEID_CPOSSESS'})) && ($modified < $vil->{'modifiedwsay'}));
			# 共鳴
			$modified = $vil->{'modifiedspsay'} if (($curpl->{'role'} == $sow->{'ROLEID_SYMPATHY'}) && ($modified < $vil->{'modifiedspsay'}));
			# 念話
			$modified = $vil->{'modifiedbsay'} if (($curpl->{'role'} == $sow->{'ROLEID_WEREBAT'}) && ($modified < $vil->{'modifiedbsay'}));
		}
